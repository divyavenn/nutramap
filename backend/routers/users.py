...
from .auth import hash_password, get_current_user, authenticate_user, create_access_token, handle_login
...
async def create_user(user: UserCreate, user_db : user_db_dependency):
    ...
    return handle_login(user_dict["email"], password)
...
def update_email(user: user_dependency, user_db : user_db_dependency):
    ...
    user_db.requirements.delete_many(query)

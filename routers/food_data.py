...
async def find_nutrient_data_for_food(db: dependency, food_id : int): 
  ...
  return [{nutrient_name: str(amt) + " " + unit} for nutrient_name, amt, unit in data]
...
async def data_for_food_by_weight(db: dependency, food_id : int, grams : float): 
  ...
  if not data:
    return "No data found."
  return [{nutrient_name: str(amount_by_weight(amt, grams)) + " " + unit} for nutrient_name, amt, unit in data]

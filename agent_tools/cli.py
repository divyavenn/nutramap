...
def _build_parser() -> argparse.ArgumentParser:
    ...
    subparsers.add_parser("whoami", help="Show current user identity")
    ...
    nutrition_parser = subparsers.add_parser("nutrition", help="Show per-component nutrition for a log entry")
    nutrition_parser.add_argument("log_id", help="Log ID (from 'foodpanel logs' output)")
    ...
    auth_sub.add_parser("whoami", help="Show current user identity (alias for top-level whoami)")
    ...
    food_top = food_sub.add_parser("top", help="Top foods by nutrient content or ratio")
...
def _print_top_foods_human(payload: Dict[str, Any], *, plain: bool = False) -> None:
    ...
    if HAVE_RICH and not plain and console is not None:
        table = Table(title=f"Top foods — {metric}")
        table.add_column("#", style="dim", justify="right")
        table.add_column("Food", style="cyan")
        table.add_column("Value", justify="right")

        for i, item in enumerate(results, 1):
            table.add_row(str(i), str(item.get("food_name", "")), str(round(item.get("value", 0), 2)))
        console.print(table)
        return

    print(f"Top foods — {metric}")
    for i, item in enumerate(results, 1):
        print(f"  {i:2}. {item.get('food_name', '')} — {round(item.get('value', 0), 2)}")
...
def main(argv: List[str] | None = None) -> int:
    ...
    try:
        ...
        with _client_from_args(args) as client:
            ...
            if args.resource == "nutrition":
                _print_json(client.get_log_nutrition(args.log_id))
                return 0
            ...
            if args.resource == "food":
                ...
                if args.action == "top":
                    payload = client.get_top_foods(
                        args.nutrient_id,
                        per_nutrient_id=args.per_nutrient_id,
                        limit=args.limit,
                    )
                    if args.json:
                        _print_json(payload)
                    else:
                        _print_top_foods_human(payload, plain=args.plain)
                    return 0
